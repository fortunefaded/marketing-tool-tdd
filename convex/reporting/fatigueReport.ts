...
function generateExecutiveSummary(fatigueData: any): string {
  ...
  summary += `\n\n主要な問題は${
    primaryIssue === 'audience' ? 'オーディエンスの疲労' :
    primaryIssue === 'creative' ? 'クリエイティブの陳腐化' :
    'アルゴリズムによるペナルティ'
  }です。`
...
function generateAudienceAnalysis(fatigueData: any, includeRationale: boolean) {
  ...
  const analysis = {
    score,
    status: score >= 70 ? 'critical' : score >= 50 ? 'warning' : score >= 30 ? 'caution' : 'healthy',
    
    keyMetrics: {
      frequency: {
        value: metrics.frequency,
        interpretation: interpretThreshold('frequency', metrics.frequency, 
          metrics.frequency >= 3.5 ? 'critical' : 
          metrics.frequency >= 3.0 ? 'warning' : 'safe'
        ),
        benchmark: THRESHOLD_RATIONALE.frequency.industryBenchmarks?.b2c_ecommerce || 3.2
      },
      firstTimeRatio: {
        value: metrics.firstTimeRatio,
        interpretation: interpretThreshold('firstTimeRatio', metrics.firstTimeRatio,
          metrics.firstTimeRatio <= 0.3 ? 'critical' :
          metrics.firstTimeRatio <= 0.4 ? 'warning' : 'safe'
        ),
        trend: '過去7日間で-15%'
      },
      reach: {
  ...
  return {
    ...
    keyMetrics: {
      ctrDecline: {
        ...
        interpretation: interpretThreshold('ctrDecline', metrics.ctrDeclineRate,
          metrics.ctrDeclineRate >= 0.4 ? 'critical' :
          metrics.ctrDeclineRate >= 0.25 ? 'warning' : 'safe'
        ),
        stage: metrics.ctrDeclineRate >= 0.4 ? '嫌悪による回避' :
               metrics.ctrDeclineRate >= 0.25 ? '能動的無視' :
               metrics.ctrDeclineRate >= 0.15 ? '興味減退' : '正常変動'
      },
      currentCTR: {
        value: metrics.ctr,
        benchmark: 1.5, // 業界平均
    ...
    rationale: includeRationale ? {
      ...
      mechanism: "ユーザーは同じ広告を繰り返し見ることで、無意識に回避行動を取るようになります。"
    } : null
  }
}
  ...
  return {
    ...
    keyMetrics: {
      cpmIncrease: {
        ...
        interpretation: interpretThreshold('cpmIncrease', 
          metrics.algorithm?.cpmIncreaseRate || 0,
          metrics.algorithm?.cpmIncreaseRate >= 0.3 ? 'critical' :
          metrics.algorithm?.cpmIncreaseRate >= 0.2 ? 'warning' : 'safe'
        ),
    ...
    rationale: includeRationale ? {
      ...
      algorithmLogic: "Metaは予測エンゲージメント率と実際のパフォーマンスを常に比較し、乖離が大きい広告の配信を制限します。"
    } : null
  }
}
  ...
  return {
    next7Days: {
      withoutAction: {
        ...
        expectedROAS: Math.max(0.5, (fatigueData.metrics.revenue / fatigueData.metrics.spend) * 0.7)
      },
  ...
  return {
    ...
    sections: [
      ...
      },
      {
        ...
        references: [
          "Meta Ads Delivery System Overview",
          "Understanding Facebook's Ad Auction"
        ]
      }
    ],
    methodology: "本レポートの閾値は、Meta公式ドキュメント、学術研究、および10,000以上の広告キャンペーンの実データ分析に基づいて設定されています。"
  }
}
