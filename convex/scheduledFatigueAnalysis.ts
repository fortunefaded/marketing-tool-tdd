  ...
  handler: async (ctx) => {
    ...
    const schedules = await ctx.db
      ...
      .filter(q => 
        q.and(
          ...
          q.lte(q.field("nextRun"), new Date().toISOString())
        )
      )
      .collect()
    ...
    for (const schedule of schedules) {
      try {
        ...
        // ユニークな広告IDを抽出
        const uniqueAdIds = [...new Set(ads
          .filter(ad => ad.ad_id)
          .map(ad => ad.ad_id!)
        )]
        
        // 各広告の疲労度を分析
        for (const adId of uniqueAdIds.slice(0, 50)) { // 一度に最大50件
...
async function analyzeAdFatigue(ctx: any, accountId: string, adId: string) {
  ...
  const fatigueData = {
    ...
    fatigueScore: {
      ...
      primaryIssue: audienceScore > creativeScore && audienceScore > algorithmScore ? 'audience' :
                    creativeScore > algorithmScore ? 'creative' : 'algorithm',
      status,
    },
    metrics: {
...
async function createFatigueAlert(ctx: any, accountId: string, adId: string, fatigueData: any) {
  ...
  const alertType = 
    fatigueData.metrics.frequency >= 3.5 ? 'frequency_exceeded' :
    fatigueData.metrics.ctrDeclineRate >= 0.25 ? 'ctr_decline' :
    fatigueData.metrics.cpmIncreaseRate >= 0.2 ? 'cpm_increase' :
    'multiple_factors'
