...
function calculateFatigueScore(metrics: any) {
  // オーディエンス疲労度（0-100）
  const audienceScore = calculateAudienceFatigueScore(metrics.frequency, metrics.firstTimeRatio)
...
async function saveFatigueTrend(ctx: any, data: any) {
  ...
  const existing = await ctx.db
    .query('fatigueTrends')
    .filter((q: any) => q.and(q.eq(q.field('adId'), data.adId), q.eq(q.field('date'), data.date)))
    .first()
...
export const getActiveAlerts = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.db
      .query('fatigueAlerts')
      .filter((q) =>
        q.and(q.eq(q.field('accountId'), args.accountId), q.eq(q.field('resolvedAt'), undefined))
      )
      .order('desc')
      .collect()
  },
