...
export const getOrders = query({
  ...
  handler: async (ctx, args) => {
    ...
    let query = ctx.db.query("ecforceOrders")
...
export const getOrderStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const firstBatch = await ctx.db.query("ecforceOrders")
      .withIndex("by_orderDate")
      .order("asc")
      .take(100)
    
    const lastBatch = await ctx.db.query("ecforceOrders")
      .withIndex("by_orderDate")
      .order("desc")
      .take(100)
...
export const getTotalOrderCount = query({
  handler: async (ctx) => {
    // 大量データの場合は推定値を返す
    // 最新1000件を取得してサンプリング
    const recentOrders = await ctx.db.query("ecforceOrders")
      .withIndex("by_orderDate")
      .order("desc")
      .take(1000)
    ...
    // 1000件以上の場合は推定
    // 最初と最後の日付から期間を計算
    const firstOrder = await ctx.db.query("ecforceOrders")
      .withIndex("by_orderDate")
      .order("asc")
      .first()
