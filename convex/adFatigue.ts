...
function calculateCTRDeclineRate(insights: any[]): number {
  ...
  const baselineCTR = baselineInsights.reduce((sum, i) => sum + (i.ctr || 0), 0) / baselineInsights.length
...
function calculateCPMIncreaseRate(insights: any[]): number {
  ...
  const baselineCPM = baselineInsights.reduce((sum, i) => sum + (i.cpm || 0), 0) / baselineInsights.length
...
export const calculateAdvancedFatigueScore = query({
  ...
  handler: async (ctx, args) => {
    ...
    // 各疲労度スコアを直接計算
    const basicAudienceScore = calculateAudienceFatigueScore(basicMetrics.frequency, basicMetrics.firstTimeRatio)
    ...
    if (args.includeInstagram) {
      ...
      const igMetrics = igInsights.length > 0 ? {
        ...
        engagement: igInsights.reduce((sum, i) => sum + ((i.clicks || 0) + (i.engagement_rate || 0) * (i.impressions || 0) / 100), 0),
      } : null
    ...
    if (algorithmMetrics.penaltyDetected) {
      algorithmScore = Math.min(100, algorithmScore + 
        (algorithmMetrics.severity === 'high' ? 40 : 
         algorithmMetrics.severity === 'medium' ? 25 : 10))
    }
    ...
    const primaryIssue = Object.entries(scores)
      .sort(([, a], [, b]) => b - a)[0][0] as FatigueType
    ...
    // 緊急対応が必要な場合の追加アクション
    if (negativeFeedback.negativeRate > 0.003) {
      recommendedAction = "緊急: ネガティブフィードバック率が危険水準（0.3%超）です。広告を即座に停止し、コンテンツを見直してください。" + recommendedAction
    }
    
    if (algorithmMetrics.penaltyDetected && algorithmMetrics.severity === 'high') {
      recommendedAction = "警告: Metaアルゴリズムによる重大なペナルティが検出されました。キャンペーンの再構築を強く推奨します。" + recommendedAction
    }
    ...
    return {
      ...
      phase2Analysis: {
        ...
        highInstagramValue: instagramValue?.totalValueScore ? instagramValue.totalValueScore > 20 : false,
...
export const saveFatigueAnalysis = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const data = {
      ...
      alertTriggered: args.fatigueScore.status === 'critical' || args.fatigueScore.status === 'warning',
    ...
    if (data.alertTriggered) {
      await ctx.db.insert("fatigueAlerts", {
        ...
        alertType: args.fatigueScore.primaryIssue === 'audience' ? 'frequency_exceeded' :
                   args.fatigueScore.primaryIssue === 'creative' ? 'ctr_decline' :
                   'cpm_increase',
        triggerMetrics: {
          frequency: args.metrics.frequency,
...
export const analyzeFatigueType = query({
  ...
  handler: async (ctx, args) => {
    ...
    scores.forEach(score => {
      ...
      const primaryIssue = Object.entries(breakdown)
        .sort(([, a], [, b]) => b - a)[0][0] as FatigueType
