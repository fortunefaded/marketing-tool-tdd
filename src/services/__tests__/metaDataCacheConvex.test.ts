...
describe('MetaDataCacheConvex', () => {
  ...
  describe('getData', () => {
    it('should fetch data from Convex', async () => {
      ...
      expect(mockConvexClient.query).toHaveBeenCalledWith(expect.any(Object), {
        accountId: 'test-account',
        startDate: '2024-01-01',
        endDate: '2024-01-31',
        limit: 1000,
  ...
  describe('findMissingDateRanges', () => {
    it('should find missing date ranges', async () => {
      ...
      const result = await cache.findMissingDateRanges('test-account', '2024-01-01', '2024-12-31')
  ...
  describe('migrateFromLocalStorage', () => {
    it('should migrate data from localStorage to Convex', async () => {
      // LZStringのモック
      vi.doMock('lz-string', () => ({
        decompressFromUTF16: vi.fn().mockReturnValue(
          JSON.stringify([
            {
              date_start: '2024-01-01',
              impressions: '1000',
