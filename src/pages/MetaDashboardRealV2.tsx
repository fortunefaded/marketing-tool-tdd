...
export const MetaDashboardRealV2: React.FC = () => {
      ...
      for (let i = 0; i < missingRanges.length; i++) {
        ...
        try {
          ...
          if ((i + 1) % 10 === 0 || i === missingRanges.length - 1) {
            ...
            console.log(`中間保存: ${allNewData.length}件 (期間 ${i + 1}/${missingRanges.length})`)
            allNewData = [] // メモリクリア
          }
  ...
  }, [accountId, activeAccount, importInsights, findMissingRanges, lastUpdateTime, syncSettings, isSyncing])
              ...
              {syncStatus && (
                <div className="text-sm text-gray-500">
                  ...
                  {syncStatus.earliestDate && syncStatus.latestDate && (
                    <div>期間: {syncStatus.earliestDate} 〜 {syncStatus.latestDate}</div>
                  )}
                  <div>Convex使用中</div>
                </div>
              )}
