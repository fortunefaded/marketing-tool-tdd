...
export const FatigueEducation: React.FC = () => {
  const [activeSection, setActiveSection] = useState<'overview' | 'metrics' | 'industries' | 'simulator'>('overview')
      ...
      {Object.entries(THRESHOLD_RATIONALE).map(([metric, data]) => (
        <div key={metric} className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 capitalize">
            {metric === 'frequency' ? 'Frequency（平均表示回数）' :
             metric === 'ctrDecline' ? 'CTR減少率' :
             metric === 'firstTimeRatio' ? '初回インプレッション比率' :
             metric === 'cpmIncrease' ? 'CPM上昇率' :
             metric === 'negativeFeedback' ? 'ネガティブフィードバック率' :
             metric === 'videoMetrics' ? '動画指標' :
             metric === 'instagramValue' ? 'Instagram価値指標' :
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                {Object.entries(data.thresholds).map(([level, value]) => (
                  <div key={level} className={`rounded-lg p-4 text-center ${
                    level === 'safe' ? 'bg-green-50 border border-green-200' :
                  ...
                  {Object.entries(data.rationale).map(([key, value]) => (
                    <div key={key} className="flex items-start">
                      <span className={`inline-block w-2 h-2 rounded-full mt-1.5 mr-2 ${
                        ...
                        key === 'warning' ? 'bg-yellow-500' : 'bg-green-500'
                      }`} />
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex justify-between text-sm text-gray-600 mt-1">
                  <span>0</span>
                  <span className="font-mono font-semibold">{simulatorValues.frequency.toFixed(1)}</span>
                  <span>10</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex justify-between text-sm text-gray-600 mt-1">
                  <span>0%</span>
                  <span className="font-mono font-semibold">{(simulatorValues.ctrDecline * 100).toFixed(0)}%</span>
                  <span>100%</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex justify-between text-sm text-gray-600 mt-1">
                  <span>0%</span>
                  <span className="font-mono font-semibold">{(simulatorValues.firstTimeRatio * 100).toFixed(0)}%</span>
                  <span>100%</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className={`p-4 rounded-lg ${
                  calculateStatus('frequency', simulatorValues.frequency) === 'critical' ? 'bg-red-50 border border-red-200' :
                  calculateStatus('frequency', simulatorValues.frequency) === 'warning' ? 'bg-yellow-50 border border-yellow-200' :
                  ...
                  <div className="text-2xl font-bold mt-1">{simulatorValues.frequency.toFixed(1)}</div>
                  <div className="text-sm text-gray-600 mt-1">
                    {calculateStatus('frequency', simulatorValues.frequency) === 'critical' ? '危険：即座の対応が必要' :
                     calculateStatus('frequency', simulatorValues.frequency) === 'warning' ? '警告：注意が必要' :
                     '安全：問題なし'}
                  </div>
                </div>

                <div className={`p-4 rounded-lg ${
                  calculateStatus('ctrDecline', simulatorValues.ctrDecline) === 'critical' ? 'bg-red-50 border border-red-200' :
                  calculateStatus('ctrDecline', simulatorValues.ctrDecline) === 'warning' ? 'bg-yellow-50 border border-yellow-200' :
                  'bg-green-50 border border-green-200'
                }`}>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="text-2xl font-bold mt-1">{(simulatorValues.ctrDecline * 100).toFixed(0)}%</div>
                  <div className="text-sm text-gray-600 mt-1">
                    {calculateStatus('ctrDecline', simulatorValues.ctrDecline) === 'critical' ? '危険：クリエイティブ変更推奨' :
                     calculateStatus('ctrDecline', simulatorValues.ctrDecline) === 'warning' ? '警告：改善を検討' :
                     '安全：正常範囲内'}
                  </div>
                </div>

                <div className={`p-4 rounded-lg ${
                  calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) === 'critical' ? 'bg-red-50 border border-red-200' :
                  calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) === 'warning' ? 'bg-yellow-50 border border-yellow-200' :
                  'bg-green-50 border border-green-200'
                }`}>
                  <div className="flex items-center justify-between">
                    ...
                    <ThresholdExplainer
                      ...
                      status={calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) as any}
                    />
                  </div>
                  <div className="text-2xl font-bold mt-1">{(simulatorValues.firstTimeRatio * 100).toFixed(0)}%</div>
                  <div className="text-sm text-gray-600 mt-1">
                    {calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) === 'critical' ? '危険：オーディエンス拡大必要' :
                     calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) === 'warning' ? '警告：新規リーチ減少中' :
                     '安全：健全な成長'}
                  </div>
                </div>
              </div>
                ...
                <p className="text-sm text-gray-700">
                  {(calculateStatus('frequency', simulatorValues.frequency) === 'critical' ||
                   ...
                   (calculateStatus('frequency', simulatorValues.frequency) === 'warning' ||
                    ...
                    calculateStatus('firstTimeRatio', simulatorValues.firstTimeRatio) === 'warning') ?
