...
export function useMemoryOptimization(options: MemoryOptimizationOptions = {}) {
  ...
  // データのサンプリング
  const sampleData = useCallback(<T>(data: T[], rate: number = sampleRate): T[] => {
  ...
  // 集計データの最適化
  const optimizeAggregatedData = useCallback((data: ECForceOrder[]): ECForceOrder[] => {
    ...
    const sortedData = [...data].sort((a, b) => 
      new Date(b.受注日).getTime() - new Date(a.受注日).getTime()
    )
  ...
  const optimizeChartData = useCallback((
    ...
    for (let i = 0; i < data.length; i += interval) {
      ...
      if (rangeData.length > 0 && typeof rangeData[0] === 'object') {
        ...
        // 数値フィールドの平均化
        const numericKeys = Object.keys(aggregated).filter(key => 
          typeof aggregated[key] === 'number'
        )
  ...
  // 大量データの分割処理
  const processInBatches = useCallback(async <T, R>(
    data: T[],
