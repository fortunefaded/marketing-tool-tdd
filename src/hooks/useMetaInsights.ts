...
interface UseMetaInsightsResult {
  ...
  findMissingRanges: (start: string, end: string) => Array<{start: string, end: string}> | undefined
}
...
export function useMetaInsights(options: UseMetaInsightsOptions): UseMetaInsightsResult {
  ...
  // 欠損期間を検出
  const findMissingRanges = useCallback((start: string, end: string) => {
  ...
  useMemo(() => {
    if (latestPageData?.items) {
      ...
      const uniqueNewInsights = newInsights.filter(i => 
        !existingIds.has(`${i.date_start}_${i.campaign_id || 'account'}_${i.ad_id || ''}`)
      )
  ...
  // データをインポート
  const importInsights = useCallback(async (data: MetaInsightsData[], strategy: 'replace' | 'merge') => {
    // バッチサイズを設定
    const batchSize = 500
    let totalImported = 0
    let totalUpdated = 0
