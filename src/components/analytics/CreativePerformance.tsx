...
export const CreativePerformance: React.FC<CreativePerformanceProps> = ({
  ...
  useEffect(() => {
    ...
    // クリエイティブタイプの分布を確認
    const typeDistribution = adLevelInsights.reduce((acc, insight) => {
                    ...
                    {metric.creative_type === 'video' && (
                      <div 
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          if (metric.video_url) {
                            setPlayingVideoId(playingVideoId === metric.creative_id ? null : metric.creative_id)
                          } else {
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs border-t pt-2">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-gray-900 font-medium">
                        {metric.impressions > 0 ? formatCurrency((metric.spend / metric.impressions) * 1000) : '¥0'}
                      </p>
                    </div>
                    <div className="text-center">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredMetrics.map((metric) => (
              <tr key={metric.creative_id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleCreativeClick(metric)}>
                    ...
                    <div className="flex-shrink-0 h-16 w-16 relative">
                      {metric.thumbnail_url || metric.creative_url ? (
                        <>
                          ...
                          />
                          {/* フォールバックアイコン（初期は非表示） */}
                          <div className="h-16 w-16 bg-gray-100 rounded flex items-center justify-center" style={{ display: 'none' }}>
                            {getCreativeIcon(metric.creative_type)}
                          </div>
                        </>
                      ) : (
                      ...
                      <span className="absolute -top-1 -right-1 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-gray-900 text-white">
                        {metric.creative_type === 'image' ? '画' : 
                         metric.creative_type === 'video' ? '動' : 
                         metric.creative_type === 'carousel' ? 'カ' : 'テ'}
                      </span>
                      {/* カルーセル枚数 */}
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                  {metric.clicks > 0 ? `${((metric.conversions / metric.clicks) * 100).toFixed(2)}%` : '-'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                  {metric.impressions > 0 ? formatCurrency((metric.spend / metric.impressions) * 1000) : '-'}
                </td>
              </tr>
            ))}
          </tbody>
