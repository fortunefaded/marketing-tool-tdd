...
export const LTVAnalysis: React.FC<LTVAnalysisProps> = ({ orders }) => {
  const ltvData = useMemo(() => {
    ...
    Object.values(customerData).forEach(customer => {
      ...
      const monthlySpend = customer.monthsSinceFirstPurchase > 0 
  ...
  const summary = useMemo(() => {
    ...
    const subscriberLTV = ltvData
      .filter(c => c.isSubscriber)
      .reduce((sum, c) => sum + c.totalSpent, 0) / (subscriberCount || 1)
    const nonSubscriberLTV = ltvData
      .filter(c => !c.isSubscriber)
      .reduce((sum, c) => sum + c.totalSpent, 0) / ((totalCustomers - subscriberCount) || 1)
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-green-600 mt-1">
                {summary.ltvMultiplier.toFixed(1)}x
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-2xl font-semibold text-blue-600">
            {ltvSegments.medium.customers}人
          </p>
          <p className="text-sm text-gray-600 mt-2">
