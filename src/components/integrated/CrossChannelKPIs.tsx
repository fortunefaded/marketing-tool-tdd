...
export const CrossChannelKPIs: React.FC<CrossChannelKPIsProps> = ({
  ...
  const kpis = useMemo(() => {
    ...
    const adOrders = ecforceOrders.filter(order => 
      order.広告URLグループ名 && order.広告URLグループ名 !== ''
    )
    ...
    // コンバージョン率
    const conversionRate = metaAdData.clicks > 0 
      ? (metaAdData.totalConversions / metaAdData.clicks) * 100 
      : 0
